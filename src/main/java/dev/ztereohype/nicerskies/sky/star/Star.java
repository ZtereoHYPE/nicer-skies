...
public class Star {
    private static final float RAD_DIFF = 0.15f;
    ...
    public Star(float randX, float randY, float randZ, float size, int color, float resizeSpeed, float spinSpeed) {
        ...
        float invsqrtDistance = Mth.invSqrt(randX * randX + randY * randY + randZ * randZ);
    ...
    public void tick(int ticks) {
        ...
        currentRadius = radius + (((float) Math.sin(ticks * resizeSpeed)) * RAD_DIFF);
    }

    // Code left for readability
    /* public void setVertices(BufferBuilder bufferBuilder) {
        float cosRot = Math.cos(currentAngle);
        float sinRot = Math.sin(currentAngle);
        ...
        for (int v = 0; v < 4; ++v) {
            ...
            bufferBuilder.addVertex(xCoord + xOffset, yCoord + yOffset, zCoord + zOffset)
                         .setColor(r, g, b, 255);
        }
    } */

    public void setVertices(BufferBuilder bufferBuilder) {
        float horizontalVertexDistance = (float) (currentRadius * Math.cos(currentAngle));
        float verticalVertexDistance = (float) (currentRadius * Math.sin(currentAngle));
